...
async function renderGTable(host) {
  ...
  if (location.hash) {
    const tr = host.querySelector(location.hash);
    if (tr) { tr.style.background = '#fdf3d7'; tr.scrollIntoView({ block: 'center' }); }
  }
}
document.querySelectorAll('.gtable').forEach(renderGTable);
