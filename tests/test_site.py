...
# Динамические маршруты справочника: их отдаёт не статический nginx, а
# node-сервис refbook-api (api-server/server.js), проброшенный host-nginx
# на /ref/api/. Статических файлов под ними нет (в отличие от api/tables/*.json),
# поэтому проверку целостности файлов на них не распространяем.
_DYNAMIC_ROUTES = ('api/calc', 'api/naca', 'api/ship', 'api/terms')
...
def test_html_integrity(path):
    ...
    for _tag, _attr, url in c.links:
        ...
        # динамические маршруты calc-API (не статические файлы) — пропускаем
        rel = os.path.relpath(fp, SITE).replace(os.sep, '/')
        if rel.startswith(_DYNAMIC_ROUTES):
            continue
